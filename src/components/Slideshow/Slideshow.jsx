      ...
      <div className="m-auto overflow-hidden">
        ...
        <div className="text-center">
          {slideImages.map((_, idx) => (
            <div key={idx} className={`inline-block h-4 w-4 rounded-full cursor-pointer mt-4 mx-2 transition-all duration-500 ${index === idx ? "bg-secondary" : "bg-primary"}`} onClick={() => {setIndex(idx)}}></div>
          ))}
        </div>
      </div>
