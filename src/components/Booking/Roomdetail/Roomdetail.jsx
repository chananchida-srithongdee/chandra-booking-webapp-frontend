...
const Roomdetail = ({ room, bookingData, setBookingData, scrollRef }) => {
   ...
   const handleChooseRoom = (e) => {
      if (submitCondition) {
         if (e.target.getAttribute('bf') === 'nobf') {
            setBookingData({ ...bookingData, breakfast: false, roomType: { type: room.type, title: room.title, price: room.price.min } });
         } else {
            setBookingData({ ...bookingData, breakfast: true, roomType: { type: room.type, title: room.title, price: room.price.max } });
         }
      }
            ...
            <div className="flex justify-between">
               <ul>
                  {room.amenities?.map((amenity, index) => (
                     <li className={styles.ameText} key={index}>
                        {amenity}
                     </li>
                  ))}
               </ul>
         ...
         <div className="sm:w-3/5 w-full flex flex-col sm:mt-0 mt-7">
            <div className={styles.detailContainer}>
               <div className="mb-7">
                  ...
                  <div className="text-md text-primary/90 flex mt-1">
                     <div className="pr-2 border-r-[1px] border-black/20">
                        {room.maxPerson} {room.maxPerson > 1 ? 'Persons' : 'Person'}
                     </div>
                     {room.beds.map(
                        (bed, index) =>
                           bed.quantity > 0 && (
               ...
               <Link to={`${roomDetailUrl}/${room.type}`} target="_blank">
                  <div className={styles.btn}>More Room Details</div>
               </Link>
            </div>
            <div className={`${styles.detailContainer} flex justify-between px-2 pt-3 xl:pb-14 h-full`}>
               <div>
                  ...
                  <div className="text-xs text-primary ml-1 mt-1">
                     Per Night
                     <br />
               ...
               <button disabled={!submitCondition} onClick={handleChooseRoom} name="nobf" className={styles.bookingbtn}>
                  Booking Now
               </button>
            </div>
            <div className={`${styles.detailContainer} flex justify-between px-2 pt-3 xl:pb-14 h-full`}>
               ...
               <button disabled={!submitCondition} onClick={handleChooseRoom} name="bf" className={styles.bookingbtn}>
                  Booking Now
               </button>
            </div>
         </div>
